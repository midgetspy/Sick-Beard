...
# Regex's to parse double-episode filenames with. Will be tried before the standard
# single-episode regexes.
config['name_parse_double_ep'] = [
    # foo_[s01]_[e01]
    re.compile('''^(.+?)[ \._\-]\[[Ss]([0-9]+?)\]_\[[Ee]([0-9]+?)\]?_\[[Ee]([0-9]+?)\]?[^\\/]*$'''),
    # foo.1x09x10 or foo.1x09-10
    re.compile('''^(.+?)[ \._\-]\[?([0-9]+)x([0-9]+)[x-]([0-9]+)[^\\/]*$'''),
    # foo.s01.e01, foo.s01_e01
    re.compile('''^(.+?)[ \._\-][Ss]([0-9]+)[\.\- ]?[Ee]([0-9]+)[\.\- ]?[Ee]([0-9]+)[^\\/]*$'''),
]
...
def processSingleName(name, verbose=False):
	...
	for r in config['name_parse_double_ep']:
		match = r.match(filename)
		if match:
			seriesname, seasno, epno1, epno2 = match.groups()

			#remove ._- characters from name (- removed only if next to end of line)
			seriesname = re.sub("[\._]|\-(?=$)", " ", seriesname).strip()

			seasno, epno1 = int(seasno), int(epno1)
