...
class IPTorrentsProvider(generic.TorrentProvider):
    ...
    def _doSearch(self, search_params, show=None):
        ...
        self.switchURL()
        return self.parseResults("{}t?99=&78=&23=&25=&65=&79=&22=&5=&q={}&qf=#torrents".format(self.url, urllib.quote(search_params)))
    ...
    def _CloudFlareError(self, response):
        if getattr(response, 'status_code', 0) in [520, 521]:
            self.session = None
            logger.log("[{}] {} Site down/overloaded cloudflare status code: {}".format(self.name, self.funcName(), response.status_code), logger.ERROR)
            return True
        return False
    ...
    def getURL(self, url, data=None):
        ...
        try:
            ...
            logger.log("[{}] {} Error loading {} URL: {}".format(self.name, self.funcName(), self.name, ex(e)), logger.ERROR)
            return None
    ...
    def _getPassKey(self):
        logger.log("[{}] {} Attempting to acquire RSS authentication details.".format(self.name, self.funcName()), logger.DEBUG)
        try:
            post_params = {
                's0': '',
                's1': '',
            ...
            (self.rss_uid, self.rss_passkey)  = re.findall(r'\/t\.rss\?u=(\d+);tp=([0-9A-Fa-f]{32});', self.getURL("{}getrss.php".format(self.url), post_params))[0]
        except:
            logger.log("[{}] {} Failed to scrape authentication parameters for rss.".format(self.name, self.funcName()), logger.ERROR)
            return False

        if not self.rss_uid:
            logger.log("[{}] {} Can't extract uid from rss authentication scrape.".format(self.name, self.funcName()), logger.ERROR)
            return False

        if not self.rss_passkey:
            logger.log("[{}] {} Can't extract password hash from rss authentication scrape.".format(self.name, self.funcName()), logger.ERROR)
            return False

        logger.log("[{}] {} Scraped RSS passkey {}".format(self.name, self.funcName(), self.rss_passkey), logger.DEBUG)
        return True
    ...
    def _doLogin(self):
        ...
        try:
            response = self.session.post("{}/take_login.php".format(self.url), data=login_params, timeout=30, verify=False)
        except (requests.exceptions.ConnectionError, requests.exceptions.HTTPError), e:
            self.session = None
        ...
        or response.status_code in [401, 403]:
            self.session = None
            logger.log("[{}] {} Login Failed, Invalid username or password for {}. Check your settings.".format(self.name, self.funcName(), self.name), logger.ERROR)
            return False

        if not self._getPassKey() or not self.rss_passkey:
            self.session = None
            logger.log("[{}] {} Could not extract rssHash info... aborting.".format(self.name, self.funcName()), logger.ERROR)
            return False
...
class IPTorrentsCache(tvcache.TVCache):
    ...
    def _getRSSData(self):
        ...
        if provider.rss_passkey:
            try:
                self.rss_url = "{}t.rss?u={};tp={};99;79;78;65;25;23;22;5;download".format(provider.ur, provider.rss_uid, provider.rss_passkey)
