...
class PushoverNotifier(INotifierPlugin):
    ...
    def notify_snatch(self, ep_name, title=notifyStrings[NOTIFY_SNATCH]):
        if self.PUSHOVER_NOTIFY_ONSNATCH:
            self._notifyPushover(title, ep_name)
            

    def notify_download(self, ep_name, title=notifyStrings[NOTIFY_DOWNLOAD]):
        if self.PUSHOVER_NOTIFY_ONDOWNLOAD:
            self._notifyPushover(title, ep_name)
    ...
    def _addMethod(self):
        def testPushover(newself, userKey=None):
            cherrypy.response.headers['Cache-Control'] = "max-age=0,no-cache,no-store"

            result = self.test_notify(userKey)
            if result:
        ...
        Home.testPushover = testPushover

    def _addStatic(self):
        app = cherrypy.tree.apps[sickbeard.WEB_ROOT]
        app.merge({
             '/images/notifiers/pushover.png': {
                'tools.staticfile.on': True,
                'tools.staticfile.filename': os.path.join(os.path.dirname(__file__), 'data/images/pushover.png'),
             },
             '/js/configPushover.js': {
                'tools.staticfile.on': True,
    ...
    def _removeMethod(self):
        if hasattr(Home, 'testPushover'):
            del Home.testPushover
    ...
    def readConfig(self, config):
        CheckSection(config, 'Pushover')
        self.USE_PUSHOVER = bool(check_setting_int(config, 'Pushover', 'use_pushover', 0))
        self.PUSHOVER_NOTIFY_ONSNATCH = bool(check_setting_int(config, 'Pushover', 'pushover_notify_onsnatch', 0))
        self.PUSHOVER_NOTIFY_ONDOWNLOAD = bool(check_setting_int(config, 'Pushover', 'pushover_notify_ondownload', 0))
        self.PUSHOVER_USERKEY = check_setting_str(config, 'Pushover', 'pushover_userkey', '')
