...
class synoIndexNotifier(INotifierPlugin):
    ...
    def update_library(self, ep_obj, add = True):
        if add:
            cur_file = ep_obj.location
            self.addFile(cur_file)
        else:
            cur_file = ep_obj
            self.deleteFile(cur_file)
    ...
    def _addStatic(self):
        ...
        app.merge({
             '/images/notifiers/synoindex.png': {
                'tools.staticfile.on': True,
                'tools.staticfile.filename': os.path.join(os.path.dirname(__file__), 'data/images/synoindex.png'),
             }
        })
    ...
    def readConfig(self, config):
        CheckSection(config, 'Synology')
        self.USE_SYNOINDEX = bool(check_setting_int(config, 'Synology', 'use_synoindex', 0))
