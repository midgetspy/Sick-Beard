...
class WebInterface:
    ...
    def comingEpisodesIcal(self):
        ...
        startdate = datetime.date(today.year, today.month, 1).toordinal()
        sql_results = myDB.select("SELECT *, tv_shows.status as show_status FROM tv_episodes, tv_shows WHERE season != 0 AND airdate >= ? AND tv_shows.tvdb_id = tv_episodes.showid", [startdate])
