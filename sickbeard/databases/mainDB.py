...
class SceneExceptions(AddAirByDateOption):
	def test(self):
	...
	def execute(self):
		...
		for tvdb_id, names in sceneExceptions.items():
			for name in names:
				self.connection.action("INSERT INTO scene_exceptions (tvdb_id, show_name) VALUES (?,?)", (tvdb_id, name))
		self.incDBVersion()
