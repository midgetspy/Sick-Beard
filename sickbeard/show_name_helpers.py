...
from sickbeard.common import countryList, Quality, SNATCHED, DOWNLOADED, WANTED
...
def isGoodResult(name, show, log=True):
    ...
    for curName in set(showNames):
        ...
        curRegex = '^(\[.+?\][ .])?' + escaped_name + '\W+(?:(?:S\d[\dE._ -])|(?:\d\d?\d?[\s.-])|(?:\d\d?\d?$)|(?:\d\d?x)|(?:\d{4}\W\d\d\W\d\d)|(?:(?:part|pt)[\._ -]?(\d|[ivx]))|Season\W+\d+\W+|E\d+\W+)'
