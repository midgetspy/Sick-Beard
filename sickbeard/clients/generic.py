...
class GenericClient(object):
    ...
    def _add_torrent_uri(self, result):
        """
        This should be overridden should return the ok from the client 
    ...
    def sendTORRENT(self, result):
        ...
        try:
            ...
            self._set_torrent_pause(result)
            
            self._set_torrent_label(result)
            
            self._set_torrent_ratio(result)
            
            self._set_torrent_path(result)

        except Exception, e:
            logger.log('Failed to send torrent to ' + self.name + ': ' + ex(e), logger.ERROR)
            return False
