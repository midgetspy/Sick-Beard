...
class PostProcessor(object):
    ...
    def process(self):
        ...
        if sickbeard.USE_SUBTITLES:
            ...
            peri = periscope.Periscope(config=False, cache_dir=sickbeard.CACHE_DIR, workers=1, multi=sickbeard.SUBTITLES_MULTI, force=False, max_depth=1, autostart=False)
            peri.languages = sickbeard.SUBTITLES_LANGUAGES
            peri.plugins = sickbeard.subtitles.getEnabledPluginList()
            peri.startWorkers()
            peri.downloadSubtitles([self.file_path])
            peri.stopWorkers()
