...
import time
from lib.jsonrpclib.jsonrpc import json
...
class NotNotNotifier:
    ...
    def _sendNotNot(self, msg, api_id=None, api_key=None, api_type=None):
        ...
        try:
            ...
            if "result" in resp:
                logger.log("NotNot notification successful.", logger.DEBUG)
                return True
            else:
                return False
        except urllib2.URLError, e:
            logger.log("NotNot notification failed." + ex(e), logger.ERROR)
            return False
    ...
    def _notifyNotNot(self, title, message, api_id, api_key ):
        """
        ...
        if not sickbeard.USE_PUSHOVER:
            logger.log("Notification for NotNot not enabled, skipping this notification", logger.DEBUG)
            return False

        logger.log("Sending notification for " + message, logger.DEBUG)

        self._sendNotNot("%s: %s" % (message, title))
        return True
