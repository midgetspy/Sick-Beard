...
class PushoverNotifier:
    ...
    def test_notify(self, userKey=None, device=None, sound=None):
        return self._sendPushover("This is a test notification from SickBeard", 'Test', userKey, device, sound )

    def _sendPushover(self, msg, title, userKey=None, device=None, sound=None):
        """
        ...
        try:
            ...
            # For HTTP status code 401's, it is because you are passing in either an invalid token, or the user has not added your service.
                ...
                subscribeNote = self._sendPushover(msg, title, userKey, device, sound )
                if subscribeNote:
    ...
    def _notifyPushover(self, title, message, userKey=None, device=None, sound=None):
        """
	...
	# if no device was given, then use the one from the config
        if not device:
            device = sickbeard.PUSHOVER_DEVICE
                      
        # if no sound was given, then use the one from the config                 
        if not sound:
            sound = sickbeard.PUSHOVER_SOUND
                     
        # if we still don't have a sound, we use the default one                                       
        if not sound:
            sound = 'pushover'

        logger.log("Sending notification for " + message, logger.DEBUG)
        self._sendPushover(message, title, userKey, device, sound)
        return True
