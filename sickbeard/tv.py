...
class TVEpisode:
    ...
    def checkForMetaFiles(self):
        ...
        if ek.ek(os.path.isfile, self.location):
            ...
            if ek.ek(os.path.isfile, ek.ek(os.path.join, self.show.location, helpers.replaceExtension(self.location, 'txt'))):
                self.hastxt = True
            else:
                self.hastxt = False
        
        # if either setting has changed return true, if not return false
        return oldhasnfo != self.hasnfo or oldhastbn != self.hastbn or oldhastxt != self.hastxt
    ...
    def createNFOs(self, epsToWrite, force=False):
        ...
        for curEpToWrite in epsToWrite:
            ...
            for tvTag in pyTivoMetadataOrder:
                ...
                if ( myEp.has_key(pyTivoMetadata[tvTag]) and (myEp[pyTivoMetadata[tvTag]]) ):
                ...
                if '|' in text:
                    ...
                    for person in people:
                        logger.log('Splitting ' + person.strip(),logger.DEBUG)
                        line += "%s : %s\n" % (tvTag, re.sub('\n', ' ', person.strip()))
                else:
                    line = "%s : %s\n" %(tvTag, re.sub('\n', ' ', text))
                    logger.log('Completed -> ' + line, logger.DEBUG)
                ...
                if metadataText:
                    ...
                    logger.log('Writing txt to ' + txtFilename, logger.DEBUG)
                    tivo_fh = ek.ek(open, txtFilename, 'w')
                    tivo_fh.write( metadataText.encode('utf-8') )
                    tivo_fh.close()
